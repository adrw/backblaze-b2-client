...
const COLLECTD_CONFIG_EJS = fs.readFileSync(
  __dirname + "/collectd/cloudron-backup.ejs",
  { encoding: "utf8" }
)
...
function injectPrivateFields(newConfig, currentConfig) {
  if (newConfig.key === exports.SECRET_PLACEHOLDER)
    newConfig.key = currentConfig.key
  if (newConfig.provider === currentConfig.provider)
    api(newConfig.provider).injectPrivateFields(newConfig, currentConfig)
}
...
function testConfig(backupConfig, callback) {
  ...
  if (!func)
    return callback(
      new BoxError(BoxError.BAD_FIELD, "unknown storage provider", {
        field: "provider"
      })
    )

  if (backupConfig.format !== "tgz" && backupConfig.format !== "rsync")
    return callback(
      new BoxError(BoxError.BAD_FIELD, "unknown format", { field: "format" })
    )

  // remember to adjust the cron ensureBackup task interval accordingly
  if (backupConfig.intervalSecs < 6 * 60 * 60)
    return callback(
      new BoxError(BoxError.BAD_FIELD, "Interval must be atleast 6 hours", {
        field: "interval"
      })
    )
...
function getByStatePaged(state, page, perPage, callback) {
  ...
  backupdb.getByTypeAndStatePaged(
    backupdb.BACKUP_TYPE_BOX,
    state,
    page,
    perPage,
...
function getBackupFilePath(backupConfig, backupId, format) {
  ...
  if (format === "tgz") {
    ...
    return path.join(
      backupConfig.prefix || backupConfig.backupFolder || "",
      backupId + fileType
    )
  } else {
    return path.join(
...
function encryptFilePath(filePath, key) {
  ...
  var encryptedParts = filePath.split("/").map(function(part) {
    ...
    return crypt
      .toString("base64") // ensures path is valid
...
function tarPack(dataLayout, key, callback) {
  ...
  var pack = tar.pack("/", {
    ...
    map: function(header) {
      ...
      if (header.size > 8589934590 || header.name > 99)
        header.pax = { size: header.size }
      return header
    },
...
function sync(backupConfig, backupId, dataLayout, progressCallback, callback) {
  ...
  const concurrency =
    backupConfig.syncConcurrency || (backupConfig.provider === "s3" ? 20 : 10)

  syncer.sync(
    dataLayout,
    function processTask(task, iteratorCallback) {
      debug("sync: processing task: %j", task)
      // the empty task.path is special to signify the directory
      const destPath =
        task.path && backupConfig.key
          ? encryptFilePath(task.path, backupConfig.key)
          : task.path
      const backupFilePath = path.join(
        getBackupFilePath(backupConfig, backupId, backupConfig.format),
        destPath
      )

      if (task.operation === "removedir") {
        debug(`Removing directory ${backupFilePath}`)
        return api(backupConfig.provider)
          .removeDir(backupConfig, backupFilePath)
          .on("progress", message => progressCallback({ message }))
          .on("done", iteratorCallback)
      } else if (task.operation === "remove") {
        debug(`Removing ${backupFilePath}`)
        return api(backupConfig.provider).remove(
          backupConfig,
          backupFilePath,
          iteratorCallback
        )
        ...
        function(retryCallback) {
          ...
          if (task.operation === "add") {
            progressCallback({
              message:
                `Adding ${task.path}` +
                (retryCount > 1 ? ` (Try ${retryCount})` : "")
            })
            debug(
              `Adding ${task.path} position ${task.position} try ${retryCount}`
            )
            var stream = createReadStream(
              dataLayout.toLocalPath("./" + task.path),
            ...
            stream.on("progress", function(progress) {
              const transferred = Math.round(
                ...
                speed = Math.round(progress.speed / 1024 / 1024)
              if (!transferred && !speed)
                return progressCallback({ message: `Uploading ${task.path}` }) // 0M@0Mbps looks wrong
              progressCallback({
                message: `Uploading ${task.path}: ${transferred}M@${speed}Mbps`
              }) // 0M@0Mbps looks wrong
            })
            api(backupConfig.provider).upload(
              backupConfig,
              backupFilePath,
              stream,
              function(error) {
                debug(
                  error
                    ? `Error uploading ${task.path} try ${retryCount}: ${error.message}`
                    : `Uploaded ${task.path}`
                )
                retryCallback(error)
              }
...
function saveFsMetadata(dataLayout, metadataFile, callback) {
  ...
  for (let lp of dataLayout.localPaths()) {
    ...
    if (emptyDirs.length)
      metadata.emptyDirs = metadata.emptyDirs.concat(
        emptyDirs
          .trim()
          .split("\n")
    ...
    var execFiles = safe.child_process.execSync(
      `find ${lp} -type f -executable\n`,
      { encoding: "utf8" }
    )
    if (execFiles === null) return callback(safe.error)

    if (execFiles.length)
      metadata.execFiles = metadata.execFiles.concat(
        execFiles
          .trim()
          .split("\n")
  ...
  if (!safe.fs.writeFileSync(metadataFile, JSON.stringify(metadata, null, 4)))
...
function checkFreeDiskSpace(backupConfig, dataLayout, callback) {
  ...
  for (let localPath of dataLayout.localPaths()) {
    ...
    let result = safe.child_process.execSync(`du -Dsb ${localPath}`, {
      encoding: "utf8"
    })
  ...
  df.file(backupConfig.backupFolder)
    .then(function(diskUsage) {
      const needed = used + 1024 * 1024 * 1024 // check if there is atleast 1GB left afterwards
      if (diskUsage.available <= needed)
        return callback(
          new BoxError(
            BoxError.FS_ERROR,
            `Not enough disk space for backup. Needed: ${prettyBytes(
              needed
            )} Available: ${prettyBytes(diskUsage.available)}`
          )
        )
...
function upload(
  ...
  debug(
    `upload: id ${backupId} format ${format} dataLayout ${dataLayoutString}`
  )
      ...
      if (format === "tgz") {
        async.retry(
          ...
          function(retryCallback) {
            ...
            tarPack(dataLayout, backupConfig.key || null, function(
              error,
              tarStream
            ) {
              ...
              tarStream.on("progress", function(progress) {
                const transferred = Math.round(
                  ...
                  speed = Math.round(progress.speed / 1024 / 1024)
                if (!transferred && !speed)
                  return progressCallback({ message: "Uploading backup" }) // 0M@0Mbps looks wrong
                progressCallback({
                  message: `Uploading backup ${transferred}M@${speed}Mbps`
                })
              })
              ...
              api(backupConfig.provider).upload(
                backupConfig,
                getBackupFilePath(backupConfig, backupId, format),
                tarStream,
                retryCallback
              )
            })
        ...
        async.series(
          [
            saveFsMetadata.bind(
              null,
              dataLayout,
              `${dataLayout.localRoot()}/fsmetadata.json`
            ),
            sync.bind(
              null,
              backupConfig,
...
function restoreFsMetadata(dataLayout, metadataFile, callback) {
  ...
  if (metadataJson === null)
    return callback(
      new BoxError(
        BoxError.EXTERNAL_ERROR,
        "Error loading fsmetadata.json:" + safe.error.message
      )
    )
  ...
  if (metadata === null)
    return callback(
      new BoxError(
        BoxError.EXTERNAL_ERROR,
        "Error parsing fsmetadata.json:" + safe.error.message
      )
    )
    ...
    function(error) {
      if (error)
        return callback(
          new BoxError(
            BoxError.EXTERNAL_ERROR,
            `unable to create path: ${error.message}`
          )
        )

      async.eachSeries(
        metadata.execFiles,
        function createPath(execFile, iteratorDone) {
          fs.chmod(
            dataLayout.toLocalPath(execFile),
            parseInt("0755", 8),
            iteratorDone
          )
        },
...
function downloadDir(
  ...
  function downloadFile(entry, done) {
    ...
    if (backupConfig.key) {
      ...
      if (!relativePath)
        return done(new BoxError(BoxError.BAD_STATE, "Unable to decrypt file"))
    }
    ...
    mkdirp(path.dirname(destFilePath), function(error) {
      ...
      async.retry(
        { times: 5, interval: 20000 },
        function(retryCallback) {
          let destStream = createWriteStream(
            destFilePath,
            backupConfig.key || null
          )
          ...
          destStream.on("progress", function(progress) {
            ...
            if (!transferred && !speed)
              return progressCallback({
                message: `Downloading ${entry.fullPath}`
              }) // 0M@0Mbps looks wrong
            progressCallback({
              message: `Downloading ${entry.fullPath}: ${transferred}M@${speed}Mbps`
            })
          })
          ...
          let closeAndRetry = once(error => {
            if (error)
              progressCallback({
                message: `Download ${entry.fullPath} to ${destFilePath} errored: ${error.message}`
              })
            else
              progressCallback({
                message: `Download ${entry.fullPath} to ${destFilePath} finished`
              })
            destStream.destroy()
            retryCallback(error)
          })

          api(backupConfig.provider).download(
            backupConfig,
            entry.fullPath,
            function(error, sourceStream) {
              ...
              progressCallback({
                message: `Downloading ${entry.fullPath} to ${destFilePath}`
              })

              sourceStream
                .pipe(destStream, { end: true })
                .on("finish", closeAndRetry)
            }
          )
        },
        done
  ...
  api(backupConfig.provider).listDir(
    backupConfig,
    backupFilePath,
    1000,
    function(entries, iteratorDone) {
      // https://www.digitalocean.com/community/questions/rate-limiting-on-spaces?answer=40441
      const concurrency =
        backupConfig.downloadConcurrency ||
        (backupConfig.provider === "s3" ? 30 : 10)
...
function download(
  ...
  debug(
    `download: Downloading ${backupId} of format ${format} to ${dataLayout.toString()}`
  )
              ...
              function(error, ps) {
                ...
                ps.on("progress", function(progress) {
                  const transferred = Math.round(
                    ...
                    speed = Math.round(progress.speed / 1024 / 1024)
                  if (!transferred && !speed)
                    return progressCallback({ message: "Downloading backup" }) // 0M@0Mbps looks wrong
                  progressCallback({
                    message: `Downloading ${transferred}M@${speed}Mbps`
                  })
                })
  ...
  download(
    ...
    function(error) {
      ...
      database.importFromFile(
        `${dataLayout.localRoot()}/box.mysqldump`,
        function(error) {
          if (error)
            return callback(new BoxError(BoxError.DATABASE_ERROR, error))
...
function downloadApp(app, restoreConfig, progressCallback, callback) {
  ...
  const appDataDir = safe.fs.realpathSync(
    path.join(paths.APPS_DATA_DIR, app.id)
  )
  if (!appDataDir) return callback(safe.error)
  const dataLayout = new DataLayout(
    appDataDir,
    app.dataDir ? [{ localDir: app.dataDir, remoteDir: "data" }] : []
  )

  const startTime = new Date()
  const getBackupConfigFunc = restoreConfig.backupConfig
    ? next => next(null, restoreConfig.backupConfig)
...
function runBackupUpload(uploadConfig, progressCallback, callback) {
  ...
  shell
    .sudo(
      `backup-${backupId}`,
      [BACKUP_UPLOAD_CMD, backupId, format, dataLayout.toString()],
      { preserveEnv: true, ipc: true },
      function(error) {
        if (
          error &&
          (error.code === null /* signal */ ||
            (error.code !== 0 && error.code !== 50))
        ) {
          // backuptask crashed
          return callback(
            new BoxError(BoxError.INTERNAL_ERROR, "Backuptask crashed")
          )
    ...
    .on("message", function(progress) {
      // this is { message } or { result }
      if ("message" in progress)
        return progressCallback({
          message: `${progress.message} (${progressTag})`
        })
...
function uploadBoxSnapshot(backupConfig, progressCallback, callback) {
  ...
  snapshotBox(progressCallback, function(error) {
    ...
    runBackupUpload(uploadConfig, progressCallback, function(error) {
      ...
      setSnapshotInfo(
        "box",
        { timestamp: new Date().toISOString(), format: backupConfig.format },
        callback
      )
    })
  })
...
function rotateBoxBackup(
  ...
  const snapshotTime = snapshotInfo.timestamp
    .replace(/[T.]/g, "-")
    .replace(/[:Z]/g, "") // add this to filename to make it unique, so it's easy to download them
  const backupId = util.format(
    "%s/box_%s_v%s",
    tag,
    snapshotTime,
    constants.VERSION
  ...
  backupdb.add(
    ...
    function(error) {
      ...
      var copy = api(backupConfig.provider).copy(
        backupConfig,
        getBackupFilePath(backupConfig, "snapshot/box", format),
        getBackupFilePath(backupConfig, backupId, format)
      )
      copy.on("progress", message =>
        progressCallback({ message: `box: ${message}` })
      )
      copy.on("done", function(copyBackupError) {
        const state = copyBackupError
          ? backupdb.BACKUP_STATE_ERROR
          : backupdb.BACKUP_STATE_NORMAL
...
function backupBoxWithAppBackupIds(
  appBackupIds,
  tag,
  progressCallback,
  callback
...
function canBackupApp(app) {
  ...
  return (
    (app.installationState === apps.ISTATE_INSTALLED &&
    ...
    app.installationState === apps.ISTATE_PENDING_UPDATE
  ) // called from apptask
}
...
function snapshotApp(app, progressCallback, callback) {
  ...
  if (
    !safe.fs.writeFileSync(
      path.join(paths.APPS_DATA_DIR, app.id + "/config.json"),
      JSON.stringify(app)
    )
  ) {
    return callback(
      new BoxError(
        BoxError.FS_ERROR,
        "Error creating config.json: " + safe.error.message
      )
    )
  }
...
function rotateAppBackup(
  ...
  var manifest = snapshotInfo.restoreConfig
    ? snapshotInfo.restoreConfig.manifest
    : snapshotInfo.manifest // compat
  const snapshotTime = snapshotInfo.timestamp
    .replace(/[T.]/g, "-")
    .replace(/[:Z]/g, "") // add this for unique filename which helps when downloading them
  const backupId = util.format(
    "%s/app_%s_%s_v%s",
    tag,
    app.id,
  ...
  backupdb.add(
    ...
    function(error) {
      ...
      var copy = api(backupConfig.provider).copy(
        backupConfig,
        getBackupFilePath(backupConfig, `snapshot/app_${app.id}`, format),
        getBackupFilePath(backupConfig, backupId, format)
      )
      copy.on("progress", message =>
        progressCallback({ message: `${message} (${app.fqdn})` })
      )
      copy.on("done", function(copyBackupError) {
        const state = copyBackupError
          ? backupdb.BACKUP_STATE_ERROR
        ...
        backupdb.update(
          backupId,
          { preserveSecs: options.preserveSecs || 0, state: state },
          function(error) {
            if (copyBackupError) return callback(copyBackupError)
            if (error) return callback(error)

            debug(
              `Rotated app backup of ${app.id} successfully to id ${backupId}`
            )
...
function uploadAppSnapshot(backupConfig, app, progressCallback, callback) {
  ...
  snapshotApp(app, progressCallback, function(error) {
    ...
    runBackupUpload(uploadConfig, progressCallback, function(error) {
      ...
      debugApp(
        app,
        "uploadAppSnapshot: %s done time: %s secs",
        backupId,
...
function backupBoxAndApps(progressCallback, callback) {
  ...
  apps.getAll(function(error, allApps) {
    ...
    async.mapSeries(
      allApps,
      function iterator(app, iteratorCallback) {
        progressCallback({
          percent: percent,
          message: `Backing up ${app.fqdn}`
        })
        percent += step
        ...
        backupAppWithTag(
          ...
          progress =>
            progressCallback({ percent: percent, message: progress.message }),
          function(error, backupId) {
      ...
      function appsBackedUp(error, backupIds) {
        ...
        backupIds = backupIds.filter(function(id) {
          return id !== null
        }) // remove apps in bad state that were never backed up

        progressCallback({
          percent: percent,
          message: "Backing up system data"
        })
        percent += step

        backupBoxWithAppBackupIds(
          backupIds,
          tag,
          progress =>
            progressCallback({ percent: percent, message: progress.message }),
...
function startBackupTask(auditSource, callback) {
  ...
  if (error)
    return callback(
      new BoxError(BoxError.BAD_STATE, `Cannot backup now: ${error.message}`)
    )
  ...
  tasks.add(tasks.TASK_BACKUP, [], function(error, taskId) {
    ...
    tasks.startTask(
      ...
      function(error, backupId) {
        ...
        eventlog.add(eventlog.ACTION_BACKUP_FINISH, auditSource, {
          taskId,
          errorMessage,
          timedOut,
          backupId
...
function ensureBackup(auditSource, callback) {
  ...
  getByStatePaged(backupdb.BACKUP_STATE_NORMAL, 1, 1, function(error, backups) {
    ...
    settings.getBackupConfig(function(error, backupConfig) {
      ...
      if (
        backups.length !== 0 &&
        new Date() - new Date(backups[0].creationTime) <
          (backupConfig.intervalSecs - 3600) * 1000
      ) {
        // adjust 1 hour
...
function cleanupBackup(backupConfig, backup, callback) {
  ...
  function done(error) {
    if (error) {
      debug(
        "cleanupBackup: error removing backup %j : %s",
        backup,
        error.message
      )
      return callback()
    ...
    api(backupConfig.provider).remove(
      backupConfig,
      path.dirname(backupFilePath),
      function(error) {
        if (error)
          debug(
            "cleanupBackup: unable to prune backup directory %s : %s",
            path.dirname(backupFilePath),
            error.message
          )
  ...
  if (backup.format === "tgz") {
    ...
    var events = api(backupConfig.provider).removeDir(
      backupConfig,
      backupFilePath
    )
    events.on("progress", function(detail) {
      debug(`cleanupBackup: ${detail}`)
    })
    events.on("done", done)
  }
}
...
function cleanupAppBackups(backupConfig, referencedAppBackups, callback) {
  ...
  backupdb.getByTypePaged(backupdb.BACKUP_TYPE_APP, 1, 1000, function(
    error,
    appBackups
  ) {
    if (error) return callback(error)
    ...
    async.eachSeries(
      appBackups,
      function iterator(appBackup, iteratorDone) {
        if (referencedAppBackups.indexOf(appBackup.id) !== -1)
          return iteratorDone()
        if (now - appBackup.creationTime < appBackup.preserveSecs * 1000)
          return iteratorDone()
        if (now - appBackup.creationTime < backupConfig.retentionSecs * 1000)
          return iteratorDone()
...
function cleanupBoxBackups(backupConfig, auditSource, callback) {
  ...
  const now = new Date()
  let referencedAppBackups = [],
    removedBoxBackups = []

  backupdb.getByTypePaged(backupdb.BACKUP_TYPE_BOX, 1, 1000, function(
    error,
    boxBackups
  ) {
    if (error) return callback(error)

    if (boxBackups.length === 0)
      return callback(null, { removedBoxBackups, referencedAppBackups })
    ...
    if (i !== boxBackups.length) {
      debug(
        "cleanupBoxBackups: preserving box backup %s (%j)",
        boxBackups[i].id,
        boxBackups[i].dependsOn
      )
    ...
    async.eachSeries(
      boxBackups,
      function iterator(boxBackup, iteratorNext) {
        ...
        if (now - boxBackup.creationTime < backupConfig.retentionSecs * 1000) {
          referencedAppBackups = referencedAppBackups.concat(
            boxBackup.dependsOn
          )
          return iteratorNext()
        }
...
function cleanupSnapshots(backupConfig, callback) {
  ...
  async.eachSeries(
    Object.keys(info),
    function(appId, iteratorDone) {
      apps.get(appId, function(error /*, app */) {
        if (!error || error.reason !== BoxError.NOT_FOUND) return iteratorDone()

        function done(/* ignoredError */) {
          safe.fs.unlinkSync(
            path.join(paths.BACKUP_INFO_DIR, `${appId}.sync.cache`)
          )
          safe.fs.unlinkSync(
            path.join(paths.BACKUP_INFO_DIR, `${appId}.sync.cache.new`)
          )
...
function cleanup(auditSource, progressCallback, callback) {
  ...
  settings.getBackupConfig(function(error, backupConfig) {
    ...
    cleanupBoxBackups(backupConfig, auditSource, function(error, result) {
      ...
      cleanupAppBackups(backupConfig, result.referencedAppBackups, function(
        error,
        removedAppBackups
      ) {
        if (error) return callback(error)
        ...
        cleanupSnapshots(backupConfig, function(error) {
          ...
          callback(null, {
            removedBoxBackups: result.removedBoxBackups,
            removedAppBackups: removedAppBackups
          })
        })
      })
    })
...
function startCleanupTask(auditSource, callback) {
  tasks.add(tasks.TASK_CLEAN_BACKUPS, [auditSource], function(error, taskId) {
    ...
    tasks.startTask(taskId, {}, (error, result) => {
      // result is { removedBoxBackups, removedAppBackups }
...
function checkConfiguration(callback) {
  ...
  settings.getBackupConfig(function(error, backupConfig) {
    ...
    if (backupConfig.provider === "noop") {
      message =
        "Cloudron backups are disabled. Please ensure this server is backed up using alternate means. See https://cloudron.io/documentation/backups/#storage-providers for more information."
    } else if (
      backupConfig.provider === "filesystem" &&
      !backupConfig.externalDisk
    ) {
      message =
        "Cloudron backups are currently on the same disk as the Cloudron server instance. This is dangerous and can lead to complete data loss if the disk fails. See https://cloudron.io/documentation/backups/#storage-providers for storing backups in an external location."
    }
...
function configureCollectd(backupConfig, callback) {
  ...
  if (backupConfig.provider === "filesystem") {
    const collectdConf = ejs.render(COLLECTD_CONFIG_EJS, {
      backupDir: backupConfig.backupFolder
    })
